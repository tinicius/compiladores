...
class Syntatic:
    ...
    def procRestoDeclaration(self):
        if self.current_token == TokenType.VARIABLE:
            self.procDeclaration()
            self.procRestoDeclaration()
        
    def procType(self):
        if self.current_token == TokenType.RESERVED_WORD_INTEGER:
            self.eat(TokenType.RESERVED_WORD_INTEGER)
        elif self.current_token == TokenType.FLOAT:
            self.eat(TokenType.FLOAT)
        elif self.current_token == TokenType.RESERVED_WORD_STRING:
    ...
    def procStmtList(self):
        if self.current_token in {TokenType.RESERVED_WORD_FOR, TokenType.RESERVED_WORD_READ, TokenType.RESERVED_WORD_WRITE, TokenType.RESERVED_WORD_READLN, TokenType.RESERVED_WORD_WRITELN, TokenType.RESERVED_WORD_WHILE, TokenType.VARIABLE, TokenType.RESERVED_WORD_IF, TokenType.RESERVED_WORD_BEGIN, TokenType.RESERVED_WORD_BREAK, TokenType.RESERVED_WORD_CONTINUE, TokenType.SEMICOLON}:
            self.procStmt()
            self.procStmtList()


    def procStmt(self):
        if self.current_token == TokenType.RESERVED_WORD_FOR:
            self.procForStmt()
        elif self.current_token in {TokenType.RESERVED_WORD_READ, TokenType.RESERVED_WORD_WRITE, TokenType.RESERVED_WORD_READLN, TokenType.RESERVED_WORD_WRITELN}:
            self.procIoStmt()
        elif self.current_token == TokenType.VARIABLE:
            self.procAtrib()
            self.eat(TokenType.SEMICOLON)
        elif self.current_token == TokenType.RESERVED_WORD_IF:
            self.procIfStmt()
        elif self.current_token == TokenType.RESERVED_WORD_BEGIN:
            self.procBloco()
        elif self.current_token == TokenType.RESERVED_WORD_BREAK:
            self.eat(TokenType.RESERVED_WORD_BREAK)
            self.eat(TokenType.SEMICOLON)
        elif self.current_token == TokenType.RESERVED_WORD_CONTINUE:
    ...
    def procEndFor(self):
        if self.current_token == TokenType.VARIABLE:
            self.eat(TokenType.VARIABLE)
        if self.current_token == TokenType.DECIMAL:
            self.eat(TokenType.DECIMAL)
        if self.current_token == TokenType.OCTAL:
    ...
    def procIoStmt(self):
        if self.current_token == TokenType.RESERVED_WORD_READ:
            self.eat(TokenType.RESERVED_WORD_READ)
            self.eat(TokenType.OPEN_PARENTHESES)
            self.eat(TokenType.VARIABLE)
            self.eat(TokenType.CLOSE_PARENTHESES)
            self.eat(TokenType.SEMICOLON)
        elif self.current_token == TokenType.RESERVED_WORD_WRITE:
            self.eat(TokenType.RESERVED_WORD_WRITE)
            self.eat(TokenType.OPEN_PARENTHESES)
            self.procOutList()
            self.eat(TokenType.CLOSE_PARENTHESES)
            self.eat(TokenType.SEMICOLON)
        elif self.current_token == TokenType.RESERVED_WORD_READLN:
    ...
    def procOut(self):
        if self.current_token == TokenType.STRING:
            self.eat(TokenType.STRING)
        elif self.current_token == TokenType.VARIABLE:
            self.eat(TokenType.VARIABLE)
        elif self.current_token == TokenType.DECIMAL: 
            self.eat(TokenType.DECIMAL)
        elif self.current_token == TokenType.OCTAL:
            self.eat(TokenType.OCTAL)
        elif self.current_token == TokenType.HEXADECIMAL:
            self.eat(TokenType.HEXADECIMAL)
        else:
            self.eat(TokenType.FLOAT)
    ...
    def procElsePart(self):
        if self.current_token == TokenType.RESERVED_WORD_ELSE:
    ...
    def procRestoAnd(self):
        if self.current_token == TokenType.OPERATOR_AND:
    ...
    def procNot(self):
        if self.current_token == TokenType.OPERATOR_NOT:
    ...
    def procRestoRel(self):
        if self.current_token == TokenType.OPERATOR_EQUAL:
            self.eat(TokenType.OPERATOR_EQUAL)
            self.procAdd()
        elif self.current_token == TokenType.OPERATOR_NOT_EQUAL:
            self.eat(TokenType.OPERATOR_NOT_EQUAL)
            self.procAdd()
        elif self.current_token == TokenType.OPERATOR_LESS:
            self.eat(TokenType.OPERATOR_LESS)
            self.procAdd()
        elif self.current_token == TokenType.OPERATOR_LESS_EQUAL: 
            self.eat(TokenType.OPERATOR_LESS_EQUAL)
            self.procAdd()
        elif self.current_token == TokenType.OPERATOR_GREATER:
            self.eat(TokenType.OPERATOR_GREATER)
            self.procAdd()
        elif self.current_token == TokenType.OPERATOR_GREATER_EQUAL:
    ...
    def procRestoAdd(self):
        if self.current_token == TokenType.OPERATOR_PLUS:
            self.eat(TokenType.OPERATOR_PLUS)
            self.procMult()
            self.procRestoAdd()
        elif self.current_token == TokenType.OPERATOR_MINUS:
    ...
    def procRestoMult(self):
        if self.current_token == TokenType.OPERATOR_MULTIPLY:
            self.eat(TokenType.OPERATOR_MULTIPLY)
            self.procUno()
            self.procRestoMult()
        elif self.current_token == TokenType.OPERATOR_DIVIDE:
            self.eat(TokenType.OPERATOR_DIVIDE)
            self.procUno()
            self.procRestoMult()
        elif self.current_token == TokenType.OPERATOR_MOD:
            self.eat(TokenType.OPERATOR_MOD)
            self.procUno()
            self.procRestoMult()
        elif self.current_token == TokenType.OPERATOR_DIVIDE_INTEGER:
            self.eat(TokenType.OPERATOR_DIVIDE_INTEGER)
            self.procUno()
            self.procRestoMult()
    ...
    def procFactor(self):
        if self.current_token == TokenType.DECIMAL:
            ...
            self.eat(TokenType.VARIABLE)
        elif self.current_token == TokenType.OPEN_PARENTHESES:
            self.eat(TokenType.OPEN_PARENTHESES)
            self.procExpr()
            self.eat(TokenType.CLOSE_PARENTHESES)
        else:
